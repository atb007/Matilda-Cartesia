#!/usr/bin/env python3
"""
Evolving Euclidean melody in C major → IAC → GarageBand.
...
Run (leave open):
  python3 live_euclidean_c_major.py
...
def maybe_mutate_pitch(step: int, degree) -> int:
    ...
    if random.random() < prob:
        return random.choice(C_MAJOR_DEGREES)
    return degree


def build_patterns():
    # Contour through C major (C D E G A …)
    base_degrees = PPingPong(PSequence([0, 2, 4, 7, 9, 7, 4, 2], 1))
    ...
    melody = PHoldOrMuteOnRest(
        degrees,
        rhythm,
        hold_prob=HOLD_ON_REST_PROB,
        initial_degree=C_MAJOR_DEGREES[0],
    )
...
def main():
    ...
    print("Live Euclidean melody — C major (ebb / flow @ 120 BPM)")
